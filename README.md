...
`docker-compose.override.yml` base compose ile **otomatik birleşir**; `docker compose up` ile gelen varsayılan mod artık canlı reload'ludur:

- **Frontend** (`frontend/src/...`): nginx yerine Vite dev server (HMR) — kaydet, tarayıcı anında güncellenir.
- **Backend** (`backend/app/...`): uvicorn `--reload` (`APP_RELOAD=1`) — `.py` kaydet, ~1 sn'de restart.
- **Worker** (`backend/app/...`): `watchmedo` `.py` değişince Celery worker'ı yeniden başlatır (`watchdog` gerekir).

Kaynak kod imaja kopyalanmaz, container'a **mount** edilir; build sadece `requirements.txt` ya da frontend `package.json` değişince gerekir:

```bash
docker compose up -d --build <servis>   # bağımlılık değiştiğinde
docker compose logs -f backend frontend worker
```

> Prod gibi (imaj içine gömülü kod, nginx) çalıştırmak için override'ı atla:
